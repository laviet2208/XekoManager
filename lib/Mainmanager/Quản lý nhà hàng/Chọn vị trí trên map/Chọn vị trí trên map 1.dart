...
class PickLocationInMap1 extends StatefulWidget {
  final accountLocation location;
  final double width;
  final String locationText;
  const PickLocationInMap1({Key? key, required this.location, required this.width, required this.locationText,}) : super(key: key);

  @override
  State<PickLocationInMap1> createState() => _PickLocationInMapState();
}

class _PickLocationInMapState extends State<PickLocationInMap1> {
  ...
  double extractData(String dateString, int index) {
    List<String> dateParts = dateString.split(',');
    if (dateParts.length == 2) {
      return double.tryParse(dateParts[index]) ?? 0;
    }
    return 0;
  }
  ...
  void initState() {
    ...
    super.initState();
    widget.location.Longitude = extractData(widget.locationText, 1);
    widget.location.Latitude = extractData(widget.locationText, 0);
