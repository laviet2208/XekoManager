...
class Themdanhmucsanpham {
  static Future<void> pushData(FoodDirectory foodDirectory) async{
    try {
      DatabaseReference databaseRef = FirebaseDatabase.instance.reference();
      await databaseRef.child('ProductDirectory').child(foodDirectory.id).set(foodDirectory.toJson());
  ...
  static Future<void> pushData1(accountShop foodDirectory, String id) async{
    try {
      ...
      await databaseRef.child('Store').child(foodDirectory.id).child('ListDirectory').set(foodDirectory.ListDirectory);
      toastMessage('Thêm danh mục thành công');
    } catch (error) {
      print('Đã xảy ra lỗi khi đẩy catchOrder: $error');
      throw error;
