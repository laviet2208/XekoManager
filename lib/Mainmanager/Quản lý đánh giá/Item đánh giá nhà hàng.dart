...
import 'package:xekomanagermain/Mainmanager/Qu%E1%BA%A3n%20l%C3%BD%20%C4%91%C3%A1nh%20gi%C3%A1/Evaluate.dart';
...
class Itemdanhgianhahang extends StatefulWidget {
  final double height;
  final double width;
  final Evaluate evaluate;
  final Color color;
  const Itemdanhgianhahang({Key? key, required this.height, required this.width, required this.evaluate, required this.color}) : super(key: key);

  @override
  State<Itemdanhgianhahang> createState() => _ItemdanhgianhahangState();
}

class _ItemdanhgianhahangState extends State<Itemdanhgianhahang> {
  ...
  void getData1() {
    final reference = FirebaseDatabase.instance.reference();
    reference.child("Restaurant/" + widget.evaluate.receiver).onValue.listen((event) {
      final dynamic orders = event.snapshot.value;
      accShop = accountShop.fromJson(orders);
      setState(() {
                    ...
                    child: RichText(
                      text: TextSpan(
                        ...
                        children: <TextSpan>[
                          ...
                          TextSpan(
                            text: accShop.name, // Phần còn lại viết bình thường
                            style: TextStyle(
                              fontSize: 16,
                              fontFamily: 'roboto',
                              fontWeight: FontWeight.normal, // Để viết bình thường
