...
import 'package:xekomanagermain/Mainmanager/Qu%E1%BA%A3n%20l%C3%BD%20voucher/Voucher.dart';
...
class Capnhatvoucher extends StatefulWidget {
  final double width;
  final double height;
  final Voucher voucher;
  final int type;
  const Capnhatvoucher({Key? key, required this.width, required this.height, required this.voucher, required this.type}) : super(key: key);

  @override
  State<Capnhatvoucher> createState() => _CapnhatvoucherState();
}

class _CapnhatvoucherState extends State<Capnhatvoucher> {
  ...
  void initState() {
    ...
    getData1();
    getData2();
    if (widget.voucher.type == 0) {
      chosenType = 'Giảm theo tiền cứng';
      Typeindex = 0;
    } else {
      chosenType = 'Giảm theo phần trăm';
      Typeindex = 1;
    }
    tenchuongtrinhcontrol.text = widget.voucher.tenchuongtrinh;
    macodecontrol.text = widget.voucher.id;
    ngaybatdaucontrol.text = widget.voucher.startTime.day.toString() + '/' + widget.voucher.startTime.month.toString() + '/' + widget.voucher.startTime.year.toString();
    ngayketthuccontrol.text = widget.voucher.endTime.day.toString() + '/' + widget.voucher.endTime.month.toString() + '/' + widget.voucher.endTime.year.toString();
    toithieugiamcontrol.text = widget.voucher.mincost.toString();
    sotiengiamcontrol.text = widget.voucher.totalmoney.toString();
    toidacontrol.text = widget.voucher.maxCount.toString();
    moikhachcontrol.text = widget.voucher.perCustom.toString();
    toidatiencontrol.text = widget.voucher.maxSale.toString();
    shop.id != '';
    area.id != '';
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text((widget.type == 1) ? 'Sửa voucher khách hàng' : 'Sửa voucher nhà hàng'),
      content: Container(
      ...
      actions: <Widget>[
        ...
        TextButton(
          ...
          onPressed: loading ? null : () async {
            ...
            if (tenchuongtrinhcontrol.text.isNotEmpty && macodecontrol.text.isNotEmpty && ngaybatdaucontrol.text.isNotEmpty && area.id != '' && toidatiencontrol.text.isNotEmpty
              ...
              if (dataCheckManager.isPositiveDouble(toithieugiamcontrol.text.toString()) && dataCheckManager.isPositiveDouble(sotiengiamcontrol.text.toString())
                ...
                Voucher voucher = Voucher(
                  ...
                  CustomList: widget.voucher.CustomList,
